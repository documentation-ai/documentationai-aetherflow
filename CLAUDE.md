...
- **`documentation.json`** — Site configuration: navigation tabs/groups, theme colors, navbar actions, SEO settings. The `navigation.tabs` array defines the entire site hierarchy.
- **`openapi.yaml`** — OpenAPI 3.0.3 spec for the AetherFlow REST API (base URL: `https://api.aetherflow.com/v1`). Referenced by `documentation.json` under the "Endpoints" group to auto-generate API reference pages.
- **`*.mdx` files (23 pages)** — Documentation content using MDX (Markdown + JSX components). Each file's `path` in `documentation.json` matches its filename without extension.
...
`documentation.json` organizes content into 6 tabs: Guides, API Reference, Pricing & Plans, Security & Compliance, Help & Support, and Changelog. When adding a new page, create the `.mdx` file and add a corresponding entry in the appropriate `navigation.tabs[].groups[].pages[]` array with `title`, `path`, and `icon` fields.
